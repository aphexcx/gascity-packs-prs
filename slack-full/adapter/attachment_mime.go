...
// --- inbound attachment media types (gp-xnc) --------------------------------
//
...
// "audio_message.m4a") — arrive with mimetype "" AND filetype "", even
// from files.info. Passing Slack's value through verbatim produced a
// payload without the key, a deterministic 422 from gc, and (before the
// coalescer's poison handling) a retry storm that blocked the channel
// (2026-08-26 09:48Z). attachmentMIMEType therefore always derives a
// value; the adapter never emits an attachment without one.
