...
def test_readme_worker_toolchain_section_carries_the_recipe_and_the_decisions() -> None:
    ...
    for clause in (
        ...
        "The marker is invalidated before the install starts and rewritten only on success",
        "only under a second, atomic reclaim lock",
        "only the pid that took a lock releases it",
        "`GC_TOOLCHAIN_LANE_DEPS_INSTALLING=<project root>`",
